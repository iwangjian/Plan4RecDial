...
def convert_data(fp, extract_kg=False, tcp_path=None):
    ...
    with open(fp, 'r', encoding='utf-8') as fr:
        for idx, line in enumerate(fr):
            ...
            # CDial-GPT needs token by token input
            input_tok_str = " ".join([w for w in input_str])
            input_tok_str = input_tok_str.replace("[ S E P ]", SEP)
            input_tok_str = input_tok_str.replace("[ U S E R ]", USER)
            input_tok_str = input_tok_str.replace("[ B O T ]", BOT)
            resp_tok_str = " ".join([w for w in resp_str])
            
            data.append([input_tok_str, resp_tok_str])
    return data
