...
def gen_query_input(token_ids, max_len, sent_b_starts, mask_id):
    ...
    dec_len = list(map(lambda i:len(token_ids[i]) - sent_b_starts[i], range(bsz)))
    ...
    tgt_pos = sum(list(map(lambda i:list(range(max_len_query * i + 1, max_len_query * i + dec_len[i])), range(bsz))), [])
...
def pad_batch_data(insts,
    ...
    """
    insts = [list(inst) for inst in insts]
    return_list = []
    max_len = max(len(inst) for inst in insts)
