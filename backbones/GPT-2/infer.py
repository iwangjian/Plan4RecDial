...
from utils.inputter_gpt2 import get_data
from utils.dataset_gpt2 import NEW_ADD_TOKENS, GPT2Dataset
...
def build_input_from_segments(history, reply, tokenizer, max_decode_length, with_eos=False, max_seq_length=512):
    ...
    user, bot = tokenizer.convert_tokens_to_ids(NEW_ADD_TOKENS)
    ...
    token_type_ids = [user] * len(sequence[1])
    is_bot = False
    for i, s in enumerate(sequence[1]):
        if s == bot:
            ...
            is_bot = False
        if is_bot:
            token_type_ids[i] = bot
    
    instance = {}
    instance["input_ids"] = list(chain(*sequence))
    instance["token_type_ids"] = [bos] + token_type_ids + [bot] * len(sequence[-1])
    return instance
